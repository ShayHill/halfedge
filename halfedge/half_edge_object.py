...
class HalfEdges(StaticHalfEdges):
    ...
    def insert_vert(self, face: Face, **vert_kwargs: Any) -> Vert:
        ...
        """
        new_vert = Vert(*face.verts, **vert_kwargs)
        try:
    ...
    def remove_edge(self, edge: Edge, **face_kwargs: Any) -> Face:
        ...
        if isinstance(pair.face, Hole):
            new_face = Hole(*{edge.face, pair.face}, **face_kwargs)
        else:
            new_face = Face(*{edge.face, pair.face}, **face_kwargs)
    ...
    def collapse_edge(self, edge: Edge, **vert_kwargs) -> Vert:
        ...
        Some ugly things can happen here than can only be recognized and avoided by
        examining the geometry. This module only addresses connectivity,
        not geometry, but I've included this operation to experiment with and use
        carefully. Can really, really break things when you aren't using a triangle
        mesh.
        """
        new_vert = Vert(edge.orig, edge.dest, **vert_kwargs)
